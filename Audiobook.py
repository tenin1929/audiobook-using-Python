...
def main():
    ...
    final_directory = os.path.join(
        current_directory, r'Text_to_speech_software')
    ...
    layout = [[sg.Text('Choose PDF File to read'), sg.Input(), sg.FileBrowse()],
              [sg.Text('Enter PDF Page number or range separated by - '),
               sg.InputText()],
                ...
                for char in values[1]:
                    if char.isdigit() == False:
                        sg.Popup("Invalid value",
                                 "Enter valid number or numbers separated by -")
                        break
                    else:
                        valid = True
                        break
    ...
    imagedirectory = glob.glob(final_directory)
    ...
    # If user wants to read range of pages
        ...
        for i in range(first_page_number-1, last_page_number):
            ...
            pix = page.getPixmap(matrix=mat)
            output = os.path.join(
                final_directory, r"image_"+str(k)+"_to_read.png")
            pix.writePNG(output)
            k += 1
    ...
    for file in os.listdir(final_directory):
        data = pytesseract.image_to_string(Image.open(
            os.path.join(final_directory, file)), lang="eng")
        # For some reason the image to text translation would put | instead of the letter I. So we replace | with I
        data = data.replace("|", "I")
        data = data.split('\n')
        mytext.append(data)
    ...
    myobj.save(os.path.join(final_directory, "audio.mp3"))
